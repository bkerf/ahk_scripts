...
- 脚本路径：F:\code\ahk_scripts\tools\clear-claude-login.ps1
- 默认会清理所有浏览器（Chrome/Edge/Brave），如需仅某一项可在命令行加 -Browsers。

例如（仅清理 Chrome）：
  powershell -NoProfile -ExecutionPolicy Bypass -File ..\tools\clear-claude-login.ps1 -Browsers Chrome -CloseBrowsers
